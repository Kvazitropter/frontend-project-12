...
const Messages = () => {
  const { username: currentUserUsername } = useSelector((state) => state.auth);
  const {
    data: messages,
        ...
        <div className="bg-light mb-4 p-3 shadow-sm small">
          <p className="m-0">
            <b>
              {isChannelsLoading
                ? 'Загрузка...'
                : `# ${activeChannel?.name ?? 'Ошибка загрузки'}`}
            </b>
          </p>
          <span className="text-muted">
            {isMessagesLoading
              ? <>Загрузка...</>
              : `${shownMessages?.length ?? 'Ошибка загрузки'} сообщений`}
          </span>
        </div>
        <div id="messages-box" className="chat-messages overflow-auto px-5 ">
          {isMessagesLoading
            ? <>Загрузка...</>
            : shownMessages?.map(({ id, body, username }) => (
          ...
          >
            <InputGroup hasValidation>
              ...
              <ButtonGroup
                ...
                type="submit"
                className="btn"
                disabled={!formik.values.message.trim() || isChannelsLoading || isMessagesLoading}
              >
